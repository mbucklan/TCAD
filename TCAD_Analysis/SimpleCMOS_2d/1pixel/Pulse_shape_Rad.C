...
void Pulse_shape_Rad(){		// name of file
...
for (double fluence=13;fluence<=15;fluence+=1){		// loop to create of graph for select bias values
	...
	TString str =TString::Format("/afs/cern.ch/work/m/mbucklan/TCAD_Analysis/SimpleCMOS_2d/1pixel/Rad_Adapted_array_bias=%.0f_Fluence=1e%.0f_200.txt",bias,fluence);		// does some magic, creates string with name of file
...
TString str2 =TString::Format("Pulse Shape for 1 Pixel 100#mum Thick Sensor at %.0fV, Resistivity=100#Omegacm",bias);		// does some magic, creates string with name of file
