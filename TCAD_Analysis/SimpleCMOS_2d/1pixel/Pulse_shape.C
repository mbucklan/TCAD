...
TGraph *gr[6];			// defines array to number of TGraphs
...
for (double bias=10;bias<=40;bias+=10){		// loop to create of graph for select bias values
	...
	TString str =TString::Format("/afs/cern.ch/user/m/mbucklan/TCAD_Analysis/SimpleCMOS_2d/1pixel/Adapted_array_Simple_Pixel_bias=%.0f_500.txt",bias);		// does some magic, creates string with name of file
...
mg->GetHistogram()->SetTitle("Pulse Shape for 1 Pixel 100#mum Thick Sensor, Resistivity=100#Omegacm");
mg->SetMinimum(0);
