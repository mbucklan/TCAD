...
void Pulse_shape_int_3pixel_Rad(){		// name of file
...
for (double fluence=2;fluence<=8;fluence+=2){		// loop to create of graph for select bias values

	TString str =TString::Format("/afs/cern.ch/work/m/mbucklan/TCAD/SimpleCMOS/DepletionSimRemesh_3pixels_Rad/Rad_Current_Cleaned_3pixel_bias=%.1f_Fluence=%.0fe+14_%.0i.txt",bias,fluence,bins);		// does some magic, creates string with name of file
	
	TString str1 =TString::Format("%.0fe+14 n_{eq} cm^{-2}",fluence);		// creates string with bias value
	...
	TString str2 =TString::Format("/afs/cern.ch/work/m/mbucklan/TCAD_Analysis/SimpleCMOS_2d/3pixel/Rad_Adapted_array_3pixel_bias=%.1f_Fluence=%.0fe+14_%.0i.txt",bias,fluence,bins);		// does some magic, creates string with name of file
	...
	TString str3 =TString::Format("/afs/cern.ch/work/m/mbucklan/TCAD_Analysis/SimpleCMOS_2d/3pixel/Rad_CC_3pixel_bias=%.1f_Fluence=%.0fe+14_%.0i.txt",bias,fluence,bins);		// does some magic, creates string with name of file
...
TString str4 =TString::Format("Colled Charge for 3 Pixel 100#mum Thick Sensor at %.0fV, Resistivity=100#Omegacm",bias);		// does some magic, creates string with name of file
